...
function initializeWristbands() {
    ...
    const registerBtn = document.querySelector('.btn.btn-register');
...
window.initWristbandsPage = function () {
    initializeWristbands();
};

function searchWristbands(searchTerm) {
    const tableRows = document.querySelectorAll('.wristbands-table tbody tr');
    const q = String(searchTerm || '').replace(/\D+/g, '');
    
    tableRows.forEach(row => {
        if (!q) { row.style.display = ''; return; }
        const numericText = row.textContent.replace(/\D+/g, '');
        if (numericText.indexOf(q) !== -1) {
            row.style.display = '';
        } else {
